...
(function () {
  ...
  function koVoiceList() {
    ...
    return all
      ...
      .filter(function (v) {
        var key = (v.name || '').trim().toLowerCase().replace(/\s+/g, ' ');
      ...
      .map(function (v, i) {
        return {
          voice: v,
          label: FRIENDLY_NAMES[i] || ('Voice ' + (i + 1)),
          system: v.name || ''
        };
      });
  }
  ...
  function currentVoice() {
    ...
    return all.filter(function (v) { return v.voiceURI === S.settings.voiceURI; })[0] || koVoice;
  }
  ...
  function voiceSection() {
    ...
    if (st.cueAudio) {
      ...
      out += '<label class="field"><span>목소리 (' + ko.length + '개)</span><select data-bind="set-voice-uri">' +
        '<option value=""' + (st.voiceURI ? '' : ' selected') + '>자동 선택</option>' +
        ko.map(function (v) {
          return '<option value="' + esc(v.voice.voiceURI) + '"' +
            (st.voiceURI === v.voice.voiceURI ? ' selected' : '') +
            ' title="' + esc(v.system) + '">' + esc(v.label) + '</option>';
        }).join('') + '</select></label>';
