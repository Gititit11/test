...
(function () {
  ...
  // 음악과 같이 틀면 안내음이 묻힌다. 동봉 mp3 는 RMS -18.5dBFS 인데
  // 스트리밍 음악은 대개 -12~-9dBFS 라 6~10dB 밀린다.
  //
  // 처음에는 tanh 로 파형을 눌러 평균을 8dB 올렸다. 커지긴 했지만 파형 자체를
  // 찌그러뜨리는 방식이라 배음이 생겨 "치지직" 하는 파열음으로 들렸다.
  // 크기를 얻는 대신 음질을 판 셈이라 접었다.
  //
  // 대신 파형을 일그러뜨리지 않는 것만 쓴다.
  //   1) 150Hz 하이패스 2단 — 말에는 거의 없고 음악에는 많은 저역을 덜어낸다.
  //      최고점이 낮아지므로 그만큼 전체를 더 키울 수 있다
  //   2) 2kHz 하이쉘프 +6dB — 말이 알아들리는 대역이자 귀가 가장 예민한 구간.
  //      전체를 키우는 것보다 여기를 키우는 쪽이 음악을 훨씬 잘 뚫는다
  //   3) 마지막에 최고점을 목표치로 맞추는 단순 곱셈
  //
  // 압축기와 리미터도 대 봤지만 이 파일들은 최고점이 뾰족하지 않아 오히려
  // 평균이 내려갔다(-13.8 → -15.0dBFS). 깎을 봉우리가 없으면 리미터는
  // 얻을 것이 없다. 그래서 쓰지 않는다.
  //
  // 웹 페이지는 다른 앱의 음악을 줄일(오디오 포커스) 수단이 없다.
  // 우리 쪽을 다듬는 것이 쓸 수 있는 전부다.
  var HP_HZ = 150;          // 하이패스 (2단 = 24dB/oct)
  var SHELF_HZ = 2000;      // 명료도 쉘프 시작
  var SHELF_DB = 6;
  var VOICE_PEAK = 0.97;    // 음량 100% 일 때 최고점. 더 키울 곳이 없는 값이다
  var BEEP_GAIN = 0.5;
  var cueGain = null;
  ...
  // 설정의 0~100% 를 이득으로. 0 이면 소리를 내지 않는다.
  function cueVolume() {
  ...
  // 저역을 덜어내고 명료도 대역을 올린 뒤 최고점을 맞춘다.
  // OfflineAudioContext 가 없으면 최고점만 맞춰 그대로 쓴다.
  function polish(raw) {
    var Off = window.OfflineAudioContext || window.webkitOfflineAudioContext;
    return audioCtx.decodeAudioData(raw.slice(0)).then(function (probe) {
      if (!Off) return normalize(probe, VOICE_PEAK);
      var off = new Off(probe.numberOfChannels, probe.length, probe.sampleRate);
      var src = off.createBufferSource();
      src.buffer = probe;
      var node = src;
      [HP_HZ, HP_HZ].forEach(function (hz) {
        var f = off.createBiquadFilter();
        f.type = 'highpass'; f.frequency.value = hz; f.Q.value = 0.707;
        node.connect(f); node = f;
      });
      var sh = off.createBiquadFilter();
      sh.type = 'highshelf'; sh.frequency.value = SHELF_HZ; sh.gain.value = SHELF_DB;
      node.connect(sh);
      sh.connect(off.destination);
      src.start();
      return off.startRendering().then(function (out) { return normalize(out, VOICE_PEAK); });
    });
  }
  ...
  function loadVoice(id) {
    ...
    return fetch(v.src)
      ...
      .then(function (raw) { return polish(raw); })
  ...
  function beep() {
    ...
    try {
      ...
      [0, 0.18, 0.36].forEach(function (t) {
        ...
        // 순수 사인파는 음악에 묻히고 사각파는 그 자체가 거칠다.
        // 삼각파는 배음이 있으면서 빠르게 줄어 귀에 부담이 없다.
        o.type = 'triangle';
