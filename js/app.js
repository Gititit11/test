...
(function () {
  ...
  // 기기가 같은 목소리를 여러 개로 보고하는 경우가 많아 이름 기준으로 하나만 남기고,
  // 시스템 명칭("Google 한국의") 대신 알아보기 쉬운 이름을 붙인다.
  var FRIENDLY_NAMES = ['Ash', 'Ivy', 'Nova', 'Sage', 'Elle', 'Mia', 'Rae', 'Vera', 'Luna', 'Iris'];

  // 안드로이드 Google TTS 는 같은 이름("한국어 (대한민국)")으로 화자를 여러 개 제공한다.
  // 이름으로 합치면 서로 다른 목소리가 사라지므로 voiceURI 로만 중복을 판단한다.
  function voiceList(all) {
    var seen = {};
    return (all || [])
      .filter(function (v) {
      ...
      .map(function (v) {
        return { voice: v, uri: v.voiceURI || v.name, system: describe(v) };
      });
  }

  // 같은 이름의 화자를 구분할 수 있게 voiceURI 에서 화자 식별자를 뽑는다
  // 예: ko-kr-x-ism-local → "한국어 (대한민국) · ism"
  function describe(v) {
    var name = v.name || '';
    var id = (v.voiceURI || '')
      .replace(/^[a-z]{2}[-_][a-z]{2}[-_]x[-_]/i, '')
      .replace(/[-_](local|network|compact|premium|enhanced)$/i, '');
    if (id && id !== v.voiceURI && id.length <= 12) return name + ' · ' + id;
    return name + (v.lang ? ' · ' + v.lang : '');
  }

  function withNames(list, offset) {
    return list.map(function (v, i) {
      var n = i + (offset || 0);
      return {
        voice: v.voice, uri: v.uri, system: v.system,
        label: FRIENDLY_NAMES[n] || ('Voice ' + (n + 1))
      };
    });
  }

  function koVoiceList(includeAll) {
    var all = [];
    ...
    var rest = withNames(voiceList(all.filter(function (v) { return !isKo(v); })), ko.length);
    return ko.concat(rest);
  }
  ...
  // 설정에서 고른 목소리가 있으면 그것을 쓴다
  function currentVoice() {
    if (!S.settings.voiceURI) return koVoice;
    var all = [];
    try { all = window.speechSynthesis.getVoices() || []; } catch (e) { return koVoice; }
    return all.filter(function (v) {
      return (v.voiceURI || v.name) === S.settings.voiceURI;
    })[0] || koVoice;
  }

  function speakWith(uri, text) {
    var synth = window.speechSynthesis;
    if (!synth || typeof window.SpeechSynthesisUtterance === 'undefined') return false;
    var all = [];
    try { all = synth.getVoices() || []; } catch (e) { all = []; }
    var v = all.filter(function (x) { return (x.voiceURI || x.name) === uri; })[0];
    try {
      synth.cancel();
      var u = new window.SpeechSynthesisUtterance(text);
      u.lang = (v && v.lang) || 'ko-KR';
      if (v) u.voice = v;
      u.rate = Number(S.settings.voiceRate) || 0.95;
      u.pitch = 1;
      u.volume = 1;
      synth.speak(u);
      return true;
  ...
  function speak(text) {
    ...
    try {
      ...
      if (v) u.voice = v;
      u.rate = Number(S.settings.voiceRate) || 0.95;
      u.pitch = 1;      // 음높이를 올리면 기계음처럼 들려서 기본값을 쓴다
      u.volume = 1;
      synth.speak(u);
      return true;
    } catch (e) { return false; }
  ...
  // ── 직접 넣은 안내 음성 ───────────────────────────────
  // <audio> 로 재생하면 안드로이드가 음악을 아예 멈출 수 있어
  // 알림음과 같은 Web Audio 경로로 내보낸다 (음악은 잠깐 작아졌다 돌아온다)
  var cueBuffer = null;
  function loadCue() {
    cueBuffer = null;
    var data = S.settings.cueAudio;
    if (!data) return;
    unlockAudio();
    if (!audioCtx) return;
    fetch(data)
      ...
      .then(function (b) { cueBuffer = b; })
      .catch(function () { cueBuffer = null; });
  }
  // 기기 음성이 마음에 들지 않을 때 원하는 목소리를 직접 녹음해 쓴다
  var recorder = null, recording = false, recChunks = [];

  function startRecording() {
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia ||
        typeof window.MediaRecorder === 'undefined') {
      alert('이 브라우저는 녹음을 지원하지 않습니다. “음성 파일 넣기”를 이용해 주세요.');
      return;
    }
    navigator.mediaDevices.getUserMedia({ audio: true }).then(function (stream) {
      ...
      recorder.onstop = function () {
        ...
        if (blob.size > 1024 * 1024) { alert('녹음이 너무 깁니다. 짧게 다시 녹음해 주세요.'); return; }
        var reader = new FileReader();
        reader.onload = function () {
          S.settings.cueAudio = reader.result;
          S.settings.cueName = '내 녹음';
          S.commit();
          loadCue();
          toast('녹음한 음성으로 안내합니다');
          render();
        };
      ...
      recorder.start();
      recording = true;
      toast('녹음 중 — 문장을 읽고 중지를 누르세요');
      render();
    }).catch(function () {
      alert('마이크를 사용할 수 없습니다. 브라우저의 마이크 권한을 확인해 주세요.');
    });
  }

  function stopRecording() {
    recording = false;
    try { if (recorder && recorder.state !== 'inactive') recorder.stop(); } catch (e) { /* 무시 */ }
    render();
  }

  function playCue() {
    if (!cueBuffer || !audioCtx) return false;
    try {
  ...
  function announceRestEnd() {
    ...
    setTimeout(function () {
      if (!playCue()) speak(REST_END_TEXT);
    }, 600);
  }
  ...
  var showAllVoices = false;

  function voiceSection() {
    var st = S.settings;
    var ko = koVoiceList(showAllVoices);

    var out = '';
    if (st.cueAudio) {
      out += '<p class="dim">내 음성을 사용 중입니다 — ' + esc(st.cueName || '파일') +
        ' (' + Math.round(st.cueAudio.length * 0.75 / 1024) + 'KB)</p>';
    } else if (!ko.length) {
      out += '<p class="dim">이 기기에 한국어 음성이 없어 기본 목소리로 읽습니다. ' +
      ...
      out += '<p class="dim">▶ 로 들어보고 마음에 드는 것을 고르세요.</p>' +
        ...
        '<label class="field inline"><span>다른 언어 목소리도 보기</span>' +
        '<input type="checkbox" data-bind="set-voice-all"' + (showAllVoices ? ' checked' : '') + '></label>';
      out += '<label class="field"><span>말하기 속도 ' + Number(st.voiceRate).toFixed(2) + '배</span>' +
        '<input type="range" min="0.7" max="1.3" step="0.05" value="' + st.voiceRate + '" data-bind="set-voice-rate"></label>';
      out += '<p class="dim">기기에 따라 목소리 품질이 크게 다릅니다. 여러 개를 들어보고 고르세요. ' +
        '안드로이드는 <b>설정 → 접근성 → 텍스트 음성 변환</b>에서 고품질 한국어 음성을 추가로 받을 수 있습니다.</p>';
    }

    out += '<div class="row gap wrap" style="margin-top:10px">' +
      '<button class="btn" data-act="voice-test">들어보기</button>' +
      '<button class="btn' + (recording ? ' danger' : '') + '" data-act="cue-record">' +
        (recording ? '■ 녹음 중지' : '● 직접 녹음') + '</button>' +
      '<button class="btn" data-act="cue-pick">음성 파일 넣기</button>' +
      (st.cueAudio ? '<button class="btn danger" data-act="cue-clear">내 음성 지우기</button>' : '') +
      '</div>' +
      '<p class="dim">기기 음성이 마음에 들지 않으면 내 목소리로 직접 녹음하거나, ' +
      '원하는 TTS로 만든 파일(mp3·m4a·wav)을 넣으세요. 넣은 소리가 기기 음성보다 우선 재생되고, ' +
      '오프라인에서도 어느 기기에서나 같은 소리가 납니다.</p>' +
      '<p class="dim">읽을 문장: “' + esc(REST_END_TEXT) + '”</p>';
    return out;
  }
  ...
  document.addEventListener('click', function (ev) {
    ...
    switch (act) {
      ...
      case 'voice-pick':
        S.settings.voiceURI = t.dataset.uri;
        S.commit();
        unlockAudio();
        speak(REST_END_TEXT);
        render();
      ...
      case 'voice-play':
        unlockAudio();
        speakWith(t.dataset.uri, REST_END_TEXT);
        break;
      case 'voice-test':
        unlockAudio();
        if (playCue()) break;
        if (!speak(REST_END_TEXT)) alert('이 브라우저는 음성 안내를 지원하지 않습니다.');
        else if (!currentVoice()) toast('한국어 음성이 없어 기본 목소리로 재생합니다');
        break;
      case 'cue-record':
        unlockAudio();
      ...
      case 'cue-pick': {
        var fi = document.createElement('input');
          ...
          var f = fi.files[0];
          if (!f) return;
          if (f.size > 1024 * 1024) { alert('1MB 이하의 짧은 음성 파일만 넣을 수 있습니다.'); return; }
          var reader = new FileReader();
          reader.onload = function () {
            S.settings.cueAudio = reader.result;
            S.settings.cueName = f.name;
            S.commit();
            loadCue();
            toast('안내 음성을 바꿨습니다');
            render();
  ...
  function onFieldChange(ev) {
    ...
    switch (bind) {
      ...
      case 'set-voice-all':
        showAllVoices = t.checked;
        render();
        break;
      case 'set-voice-uri':
        S.settings.voiceURI = t.value;
        S.commit();
        if (ev.type === 'change') { unlockAudio(); speak(REST_END_TEXT); }
        break;
      case 'set-voice-rate':
        S.settings.voiceRate = Math.min(1.3, Math.max(0.7, num(t.value, 0.95)));
        S.commit();
        if (ev.type === 'change') { unlockAudio(); speak(REST_END_TEXT); render(); }
        break;
      case 'set-weight-kg':
  ...
  restoreRest();
  render();
  if (S.settings.cueAudio) {
    document.addEventListener('pointerdown', loadCue, { once: true });
  }
})();
