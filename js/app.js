...
(function () {
  ...
  function voiceSection() {
    ...
    var out = '';
    if (st.cueAudio) {
      out += '<p class="dim">직접 넣은 음성 파일을 사용 중입니다 — ' + esc(st.cueName || '파일') +
      ...
      out += '<label class="field"><span>목소리 (' + ko.length + '개)</span><select data-bind="set-voice-uri">' +
        ...
        ko.map(function (v) {
          return '<option value="' + esc(v.voiceURI) + '"' +
            (st.voiceURI === v.voiceURI ? ' selected' : '') + '>' + esc(v.name) + '</option>';
        }).join('') + '</select></label>';
    ...
    out += '<div class="row gap wrap" style="margin-top:10px">' +
      ...
      (st.cueAudio ? '<button class="btn danger" data-act="cue-clear">파일 지우기</button>' : '') +
      '</div>' +
      '<p class="dim">원하는 목소리로 직접 녹음하거나 다른 TTS 서비스에서 만든 파일(mp3·m4a·wav)을 ' +
      '넣으면 그 소리로 안내합니다. 기기 음성보다 확실히 자연스럽고, 어느 기기에서나 같은 소리가 납니다.</p>';
    return out;
  }
