...
(function () {
    ...
    return '' +
      '<div class="page">' +
        '<div class="card">' +
          ...
          '<p class="dim"><b>앱을 보고 있는 동안</b> 정확한 시각에 불러 줘요. ' +
            '화면에서 내려놓으면 브라우저가 앱을 잠재워서 그 사이에는 부르지 못해요 ' +
            '— 아래에서 캘린더에 넣어 두세요.</p>' +
        '</div>' +
        permNote +
  ...
  function calCard() {
    return '<div class="card cal">' +
      '<h3>⏰ 앱을 닫아도 울리게 하기</h3>' +
      '<p class="dim">마중물을 <b>화면에서 내려놓기만 해도</b> 그 사이에는 부르지 못해요. ' +
  ...
  function diagCard() {
    ...
    rows.push(check(true, '앱을 보고 있을 때', '정확한 시각에 알려 줘요'));
    rows.push(check(false, '내려놓거나 닫았을 때',
      '브라우저가 앱을 잠재워서 그 사이에는 못 알려요. 앱을 다시 열면 그때 밀린 알림을 알려 줘요' +
      (d.occasionalWake ? ' (가끔 스스로 깨어나 알리기도 하지만 시각은 믿을 수 없어요)' : '')));
    ...
    return '<div class="card diag">' +
      ...
      '<p class="dim">웹앱은 브라우저에 "이 시각에 깨워 줘" 를 예약해 둘 수 없어요. ' +
        '알릴 수 있는 건 앱 자신뿐인데, 화면에서 내려가면 브라우저가 앱을 잠재워요. ' +
        '그래서 정확한 시각에 확실히 받으려면 위의 <b>캘린더 등록</b>이 답이에요.</p>' +
    '</div>';
  }
